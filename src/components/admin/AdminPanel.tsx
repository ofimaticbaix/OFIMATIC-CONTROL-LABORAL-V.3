...
import { Clock, Loader2, Search, Calendar, ArrowRight, LayoutDashboard, FileText, RefreshCcw, ShieldAlert } from 'lucide-react';
...
export const AdminPanel = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      // Intentamos cargar con perfiles, si falla, cargamos solo entradas
      ...
      // Notificación silenciosa en lugar de bloqueo
      toast({ 
        variant: 'destructive', 
        title: 'Acceso Restringido', 
        description: 'No tienes permisos para ver datos de otros empleados.' 
      });
    } finally {
      setLoading(false);
    }
  ...
  if (loading) return (
    <div className="h-[60vh] flex flex-col items-center justify-center gap-4">
      ...
      <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400">Verificando credenciales...</p>
    </div>
  );
      ...
      <div className="relative overflow-hidden rounded-[2.5rem] bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl border border-slate-200/60 dark:border-slate-800 shadow-2xl min-h-[450px]">
        ...
        {entries.length === 0 ? (
          <div className="p-24 flex flex-col items-center justify-center text-center space-y-4">
            ...
            <div className="space-y-1">
              <p className="font-bold text-slate-900 dark:text-white uppercase text-xs tracking-widest">Acceso Bloqueado</p>
              <p className="text-[10px] text-slate-500 uppercase max-w-[250px] mx-auto">
                La base de datos deniega la lectura. Aplica el SQL de limpieza en Supabase.
              </p>
            </div>
