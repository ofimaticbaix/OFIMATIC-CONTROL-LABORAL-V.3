...
import { 
  ...
  LayoutDashboard, FileText, AlertCircle, RefreshCcw, MapPin 
} from 'lucide-react';
...
export const AdminPanel = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      // Consulta con unión explícita a perfiles
      ...
      toast({ 
        ...
        description: 'Verifica las políticas RLS en Supabase.' 
      });
    } finally {
      setLoading(false);
    }
  ...
  if (loading) return (
    <div className="h-[60vh] flex flex-col items-center justify-center gap-4">
      ...
      <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400">Accediendo a la base de datos...</p>
    </div>
  );
      ...
      <Tabs defaultValue="registros" className="w-full space-y-6">
        <div className="px-2">
          <TabsList className="bg-white/40 dark:bg-slate-900/40 backdrop-blur-md border border-white/50 dark:border-slate-700/50 rounded-2xl p-1 h-12 shadow-sm">
            <TabsTrigger value="registros" className="rounded-xl px-8 py-2 data-[state=active]:bg-white dark:data-[state=active]:bg-slate-800 data-[state=active]:text-blue-600 font-bold uppercase text-[10px] tracking-widest transition-all gap-2">
            ...
            {entries.length === 0 ? (
              <div className="p-20 flex flex-col items-center justify-center text-center space-y-4">
                ...
                <div className="space-y-1 text-slate-400">
                  <p className="font-bold uppercase text-xs tracking-widest">Sin acceso a los datos</p>
                  <p className="text-[10px] uppercase">Asegúrate de haber ejecutado el SQL en Supabase</p>
                </div>
              </div>
            ) : (
